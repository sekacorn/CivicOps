...
public class RefreshTokenService {
  ...
  public RotatedRefreshToken rotate(String rawToken) {
    ...
    if (!current.isUsableAt(now) || !current.getUser().isActive()) {
      throw invalid(
          current.getRevokedAt() != null ? "REVOKED_REFRESH_TOKEN" : "EXPIRED_REFRESH_TOKEN");
    }
  ...
  private IssuedRefreshToken create(User user) {
    ...
    Instant now = Instant.now(clock);
    RefreshToken entity =
        new RefreshToken(user, hash(raw), now.plus(properties.refreshTokenTtl()), now);
  ...
  private AuthenticationFailedException invalid(String code) {
    return new AuthenticationFailedException(
        code, "The refresh token is invalid, expired, or revoked");
  }
