...
public class OrganizationService {
    ...
    public OrganizationResponse create(CreateOrganizationRequest request) {
        Organization organization = new Organization(request.name().trim(), clean(request.legalName()),
                clean(request.description()), request.organizationType(), normalizeEmail(request.email()),
                clean(request.phone()), clean(request.website()), clean(request.addressLine1()),
                clean(request.addressLine2()), clean(request.city()), clean(request.state()),
                clean(request.postalCode()), normalizeCountry(request.country()));
