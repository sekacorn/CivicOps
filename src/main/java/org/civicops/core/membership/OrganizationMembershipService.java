...
public class OrganizationMembershipService {
    ...
    public OrganizationMembershipService(OrganizationMembershipRepository memberships,
                                         OrganizationService organizations, UserService users, Clock clock) {
    ...
    public MembershipResponse create(UUID organizationId, CreateMembershipRequest request) {
        if (!request.role().isOrganizationAssignable()) {
            throw new BusinessRuleException("INVALID_ORGANIZATION_ROLE", "SYSTEM_ADMIN is a platform role and cannot be assigned through an organization membership");
        }
        if (memberships.existsByOrganizationIdAndUserId(organizationId, request.userId())) {
            throw new ConflictException("DUPLICATE_MEMBERSHIP", "The user already has a membership in this organization");
        }
        ...
        if (!organization.isActive() || !user.isActive()) {
            throw new BusinessRuleException("INACTIVE_MEMBERSHIP_PARTY", "Memberships require an active organization and user");
        }
        OrganizationMembership membership = new OrganizationMembership(organization, user, request.role(), Instant.now(clock));
    ...
    public List<MembershipResponse> listForOrganization(UUID organizationId) {
        ...
        return memberships.findAllByOrganizationId(organizationId).stream().map(MembershipResponse::from).toList();
    }
}
