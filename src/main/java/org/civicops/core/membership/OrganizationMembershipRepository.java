...
public interface OrganizationMembershipRepository
  ...
  Optional<OrganizationMembership> findByOrganizationIdAndUserIdAndActiveTrue(
      UUID organizationId, UUID userId);

  List<OrganizationMembership> findAllByOrganizationId(UUID organizationId);

  @Query(
      "select membership from OrganizationMembership membership join fetch membership.organization "
