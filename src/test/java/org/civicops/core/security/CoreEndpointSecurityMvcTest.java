...
class CoreEndpointSecurityMvcTest {
  ...
  void invalidTokenReturns401() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/organizations/{id}/memberships", UUID.randomUUID())
  ...
  void validAccessTokenAuthenticatesAndAuthorizesRequest() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/organizations/{id}/memberships", organizationId)
  ...
  void crossOrganizationRequestReturns403() throws Exception {
    ...
    doThrow(new AccessDeniedException("no membership"))
        .when(access)
        .requireRole(userFromOrganizationA, organizationB, Role.ORG_ADMIN);

    mvc.perform(
            get("/api/v1/organizations/{id}/memberships", organizationB)
